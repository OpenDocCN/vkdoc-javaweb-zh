...
最后，`spring-boot-maven-plugin`包含将应用程序打包成可执行的 JAR/WAR 并运行它的目标。
...
下一步是启动并运行我们的应用程序。为此，打开命令行，导航到`hello-rest`文件夹，并运行以下命令:
...
安装完成后，您可以使用浏览器右上角的 RESTClient 图标![img/332520_2_En_3_Figa_HTML.jpg](img/332520_2_En_3_Figa_HTML.jpg)启动 RESTClient。图 [3-14](#Fig14) 显示了 RESTClient 应用程序对我们的 Greet 服务的请求以及相应的响应。
