...
![images](img/square.jpg) **警告**如果您不打算为应用程序验证创建额外的数据库实例，则不要发出本节中的任何命令。可以直接跳到*测试增强安全性*；但是，要了解如何创建和强化 Oracle 数据库实例以及数据库链接，请务必通读本节。
...
![images](img/square.jpg) **注意**这将只在`ORCL`实例上进行，并且只有在您安装第二个数据库实例`apver`来进行应用程序验证时才进行。
...
![images](img/square.jpg) **注意**以下内容假设您正在一个单独的数据库实例`apver`上运行应用程序验证。如果您不是，那么唯一的区别是您将能够作为`appsec`用户连接到`orcl`实例——您不必作为`SYS`连接。
...
![images](img/square.jpg) **注意**如果您没有创建专用于应用程序验证的额外 Oracle 数据库实例，则不要更改代码。您不需要更新编码的连接字符串`prime`。
...
![images](img/square.jpg) **注意**正是这个值`prime`指导我们的应用程序使用备用数据库实例进行应用程序验证。
