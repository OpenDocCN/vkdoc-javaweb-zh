...
![images](images/square.jpg) **注意**在 Windows XP 中，你会用*文档和设置*代替这些命令中的*用户*。
...
![images](images/square.jpg) **注意**将`WALLET_OVERRIDE`指令放在**服务器** *sqlnet.ora* 文件中(例如*% ORACLE _ HOME % \ NETWORK \ ADMIN \ sqlnet . ora*)可以阻止 ORACLE 数据库响应客户端连接。我的建议是，如果您在与 Oracle 数据库相同的计算机上测试客户机 wallet，那么您可以在没有 *sqlnet.ora* 文件中的`WALLET_OVERRIDE`的情况下启动数据库，然后在测试客户机 wallet 时临时添加该指令*。*
...
![images](images/square.jpg) **注意**至少主机名在你的情况下会有所不同，所以修改那些设置。
...
![images](images/square.jpg) **警告**对于 Oracle client wallet 文件，攻击者可以使用您在任何实例上配置的指定用户进行连接，只要该用户存在并具有相同的密码。
...
![images](images/square.jpg) **警告**当您完成当前问题的故障排除后，请确保禁用跟踪日志记录。它会生成大量文件，这些文件可能包含大量数据，这些数据不仅会占用大量磁盘空间，还会带来安全隐患。在更高级别的跟踪日志记录中，从查询返回的数据也会显示在跟踪文件中。
...
![images](images/square.jpg) **注意**你会在*chapter 11/wallet/Oracle log . properties .*找到一个完整的配置属性文件
...
![images](images/square.jpg) **注意**你会在一个名为 *Chapter11/AppSec.sql* 的文件中找到这个 SQL 脚本。
...
![images](images/square.jpg) **注意**我们刚刚把这个过程变成了特定应用程序的特定版本所独有的(呈现内部类的那个)。
...
对于我们将要使用的加密算法，我们还需要一个 16 字节的`RAW`初始化向量(IV)。我们将再次通过使用应用程序内部类名作为 IV 的一部分，使这个函数*特定于应用程序*。参见[清单 11-21](#list_11_21) 。实际上，我们将`class_name`与字符串“SufficientLength”连接起来，将其转换为`RAW`，并获得前 16 个字节作为 IV。
...
![images](images/square.jpg) **注意**这段代码可以在*chapter 11/orajavsec/Oracle javasecure . Java .*中找到
...
现在是时候区分普通应用程序用户和管理应用程序用户了。我们需要第二个用户，我称之为`OSADMIN`。这些用户之间的区别将在下一节通过单个角色授权来说明。增加该角色的原因是管理 Oracle 数据库上应用程序的连接字符串列表的更新。
...
请注意，这个方法被指定为`private`——您可能希望在`OracleJavaSecure`中使用另一个方法来建立 FTP 连接并将连接返回给客户端应用程序，而不是将明文 FTP 密码返回给应用程序。我们不想把我们的密码给应用程序。
...
我现在要描述一些揭示我们想要追求安全的程度的东西。如果我们将应用程序验证任务与实际的应用程序数据分离开来会怎么样？那会给我们带来什么安全？我们将实现的主要优势如下:
...
在本节中，我们将创建一个新的数据库实例，可能是在同一台服务器上。此实例将有足够的特权来完成应用程序验证，但仅此而已。为了确保这一点，我们不会创建我们已经讨论过的特权角色，`secadm_role`和`app_sec_role`；相反，我们将作为`SYS`用户完成所有的配置步骤。

![images](images/square.jpg) **警告**如果您不打算为应用程序验证创建额外的数据库实例，则不要发出本节中的任何命令。可以直接跳到*测试增强安全性*；但是，要了解如何创建和强化 Oracle 数据库实例以及数据库链接，请务必通读本节。

我们的应用验证呢，`appver`用户密码？它还容易被窥探吗？嗯，它没有被加密，任何人只要够勤奋就可以恢复我们的混淆的`OJSCode`类。因此，我们需要问的问题是，密码泄露会给我们带来什么样的安全风险。

我们可能会拍拍自己的背，说我们已经用调用`p_appver_logon`存储过程的`appver`模式上的登录触发器`t_screen_appver_access`覆盖了它。我们可能会对那些认为他们可以通过 SSO 代理要求，以及我们的双因素身份验证、加密密钥交换和应用程序验证过程的人嗤之以鼻。
...
如果我们想知道应用程序安全用户使用的所有 Oracle 过程的名称，该怎么办？我们可以查询`ALL_PROCEDURES PUBLIC`视图:
...
我们需要大约 2 GB 的硬盘空间来创建一个足够大的数据库，以保存我们进行应用程序验证所需的内容。“为什么这么多？”你可能会问。请记住，要进行应用程序验证，我们需要一个基本的 Oracle 数据库，我们需要数据字典视图，我们需要运行 PL/SQL，我们需要运行 Java。除此之外，对于双因素身份验证，我们需要发送电子邮件，并且需要配置系统权限来读取网络上的数据(URL)。所有这些功能都需要空间。
...
每个 Oracle 数据库实例通常在系统重启时由服务启动。你可以通过进入**开始**菜单并运行电脑管理应用程序，在 Windows 中查看这些服务。你需要使用系统管理员权限。转到服务和应用程序，然后转到服务，向下滚动到 Oracle 服务。它们通常都以前缀“Oracle”命名，并按字母顺序排序。我们不打算探索在 Unix 或 Linux 上创建、启动或停止进程；步骤相同，但是命令(运行命令级文件)不同。
...
![images](images/square.jpg) **注意**这里我们会得到一个无法启动服务的错误。没关系，因为我们还没有真正创建数据库，但是现在启动它的服务已经配置好了。
...
![images](images/square.jpg) **注**该命令包含在一个名为 *Chapter11/apver/ `ApVerDBCreate.sql.`* 的文件中
...
![images](images/square.jpg) **注意**这些命令可以在文件*chapter 11/apver/apversys . SQL*中找到
...
![images](images/square.jpg) **注意**这是安全步骤一。在完成这一步之前，请不要继续。
...
![images](images/square.jpg) **注意**这些假脱机日志文件将被创建在命令提示符的当前目录中。

##### 增加流程数量

您可能还记得在 *init.ora* 文件中看到过 150 个进程的标准设置。这对并发 Oracle 连接设置了限制。我们希望处理大量的并发连接来进行应用程序验证。想象一下，每个人在周一早上开始工作并登录一个或多个我们的安全 Oracle 应用程序。我们可以轻松地超过 150 个并发连接。

还记得我们为应用程序验证配置了一个特殊的概要文件，`appver`用户，`appver_prof`。对于该配置文件，我们将`SESSIONS_PER_USER`设置为无限制。但是，我们注意到实际的限制是由进程的数量控制的。让我们增加流程的数量。

首先以`SYS`的身份连接，并输入我们刚刚设置的新的`SYS`密码。您可以通过下面显示的第一种语法使用`TNSlistener`服务进行连接，或者像第二种语法一样直接连接到数据库，只要您的`ORACLE_SID`环境变量设置为`apver`。在这两种情况下，您可能希望从 Windows 计算机管理应用程序启动侦听器服务。
...
发送邮件的能力不是 Oracle 数据库自动包含的特性。您还记得，当我们实现双因素身份验证时，我们将该特性添加到了原始的`orcl`实例中。我们也将把它添加到`apver`实例中。事实上，我们现在将把双因素身份认证作为应用程序验证的一项功能，而不是与我们建立的每个 Oracle 连接相关联。
...
许多脚本用于配置和启用 Oracle JVM。我们将只运行其中的两个，即我们的应用程序验证安全流程所需的两个。在通常列出的将 Java 构建到 Oracle 数据库的五个脚本中，我们将只执行 *initjvm.sql* 和 *catjava.sql* 。同样，我们选择的理由是我们将只启用我们需要的功能，从而避免一些潜在的安全弱点。
...
关闭数据库，从计算机管理应用程序重启`TNSListener`和数据库服务(在开始菜单/运行命令中键入计算机管理):
...
您可能希望能够从远程 GUI 管理应用程序(如 TOAD)或任何远程应用程序(包括 SQL*Plus)连接到作为`SYS`的`apver`实例；但是，使用`AS SYSDBA`特权远程连接到数据库实例的能力受到限制。为了使连接成功，数据库实例必须存在远程登录密码文件。为了作为`SYS AS SYSDBA`远程登录到`apver`实例，我们需要为`apver`创建远程登录密码文件。
...
我们现在应该能够从本地命令提示符或远程会话连接到作为`SYS`的`apver`实例。继续以`SYS AS SYSDBA`的身份连接，这样我们就可以创建应用程序验证所需的结构。

![images](images/square.jpg) **注意**这些命令可以在文件*chapter 11/apver/newsys . SQL .*中找到

创建一个与我们在`orcl`实例上创建的角色相同的`create_session_role`角色，然后创建`appsec`用户。参见[清单 11-46](#list_11_46) 。一定要给`appsec`一个复杂的密码。还要在默认表空间上为`appsec`提供足够大的`QUOTA`，以便保存用于应用程序验证的结构和数据。
...
正如我在本节开始时提到的，我们将配置最低限度的配置，以便在这个新的`apver`实例中进行应用程序验证。出于这个原因，我们将依靠`SYS`用户来配置我们所有的结构，甚至是那些在`appsec`模式中的结构。好吧，除了一件事，我们最好现在就离开。

我们希望创建一个数据库链接，供`appsec`结构使用，特别是进行双因素认证。回想一下，我们已经在`HR.EMPLOYEES`表中存储了一个电子邮件地址，并创建了另一个表，其中包含用于在`HR`模式中进行双因素身份验证的地址，`emp_mobile_nos`。但是，这些表在不同的 Oracle 实例上，`orcl`。作为我们在`apver`实例上进行的应用程序验证过程的一部分，我们需要通过数据库链接读取这些表。
...
![images](images/square.jpg) **注意**这些命令可以在文件*chapter 11/apver/newappsec . SQL .*中找到
...
我们现在返回到`SYS`用户，以便在新的`apver`实例上完成我们的应用程序安全结构的大部分安装。
...
我们建立一个单独的 Oracle 实例来进行应用程序验证/授权的主要原因之一是，如果黑客获得了对`appver`密码的访问权限，我们希望对黑客能够看到的内容和做的事情进行更严格的限制。请记住，我们只是混淆了密码；我们还没有加密。
...
![images](images/square.jpg) **注意**如果这正在生产中使用，您将在撤销不适当的更宽权限之前授予适当的窄权限(如图所示),以便在更新期间保持适当的功能工作。

#### 为应用程序授权创建剩余的结构

用于`apver`实例的 *NewSys.sql* 脚本的剩余部分配置了我们进行应用程序授权所需的所有结构。大多数结构都是在`appsec`模式中创建的。作为`SYS`用户，我们只需在我们正在创建的结构名前面加上模式名`appsec.`
...
![Images](images/t1102a.jpg)

![images](images/square.jpg) **注意**因为我们没有在`apver`实例上创建`secadm`用户，所以我们不能在`secadm`模式中创建`t_screen_appver_access`触发器。我们将在`appsec`模式中创建它。

脚本中有几个地方需要用真实的操作系统用户 ID(即 Windows 登录名)替换占位符。搜索并替换`OSUSER`和`OSADMIN`。`OSUSER`是一个人(或多人)想要运行我们的安全应用程序。`OSADMIN`是一个像您一样的人，他需要连接以便在数据库中注册应用程序连接字符串。
...
此时，我们不再需要在`ORCL`实例中进行应用程序验证。我们将通过应用程序验证，禁用`appver`用户到`ORCL`的连接。使用以下命令，以`appsec`或 SYS 用户的身份从 SQL 客户端连接到`ORCL`实例。

![images](images/square.jpg) **注意**这将只在`ORCL`实例上进行，并且只有在您安装第二个数据库实例`apver`来进行应用程序验证时才进行。

`ALTER USER appver PASSWORD EXPIRE;`

### 测试增强的安全性

我们现在可以测试我们已经建立的一切，包括在单独的 Oracle 实例`apver`上隔离应用程序身份验证。我们将分两部分进行测试:首先使用`OracleJavaSecure`中的`main()`方法，然后使用单独的应用程序`testojs.TestOracleJavaSecure`。

再次确认您已经在*OracleJavaSecure.java*的顶部设置了适合您组织的域名和其他地址。还要确保文件顶部的 SQL 命令已经被注释。(SQL 命令在其他*的顶部。本章中的 java* 文件也应该保留注释以便编译。)

![images](images/square.jpg) **注意**以下内容假设您正在一个单独的数据库实例`apver`上运行应用程序验证。如果您不是，那么唯一的区别是您将能够作为`appsec`用户连接到`orcl`实例——您不必作为`SYS`连接。
...
![images](images/square.jpg) **注意**如果您没有创建专用于应用程序验证的额外 Oracle 数据库实例，则不要更改代码。您不需要更新编码的连接字符串`prime`。
...
![images](images/square.jpg) **注意**正是这个值`prime`指导我们的应用程序使用备用数据库实例进行应用程序验证。

#### 编辑要使用的应用程序密码

我们将为`HRVIEW`应用程序向 Oracle 数据库上传一个连接字符串列表。更新`OracleJavaSecure`的`main()`方法，使`HR`和`appusr`用户拥有正确的密码(替换“密码”字符串)，如[清单 11-55](#list_11_55) 所示。还要更正 Oracle 应用程序连接字符串的其他方面。

![images](images/square.jpg) **注意**这些连接字符串旨在连接到`orcl`实例，而不是新的`apver`实例。

***清单 11-55。**应用程序的连接字符串，OracleJavaSecure.main()*
...
您可以看到，当我们从`apver`实例获得该应用程序的 Oracle 连接列表时，该列表是空的，`connsHash.size = 0`。我们能够将连接字符串插入列表并使用它们。使用其中一个连接字符串，我们查询数据库以获得`SYSDATE`。

当我们再次运行这个命令时，我们看到一会儿是我们的`connsHash.size = 2`，一会儿是`= 1`。第一个值是我们从 Oracle 获得的这个应用程序的列表中连接字符串的数量。第二个值是我们呼叫`removeAppConnString()`后的号码。我们立即调用`putAppConnString()`两次来添加和覆盖连接字符串，然后我们调用`putAppConnections()`来为这个应用程序在 Oracle 中存储两个连接字符串的列表。我们再次使用其中一个从 Oracle 获取`SYSDATE`。
...
我们将再次编辑*OracleJavaSecure.java*，以测试我们从旧版本的应用程序中复制连接字符串列表到新版本的能力。打开文件并编辑内部类`InnerRevLvlClass`(靠近顶部)，增加版本号，例如从`20110101a`到`20110101b`。见[清单 11-56](#list_11_56) 。
...
![images](images/square.jpg) **注意**我们不必在 Oracle 数据库上创建这个 java 代码的新版本，因为 serialVersionUID 没有改变。
...
作为`apver`上的`SYS`用户，通过查询`appsec.v_app_conn_registry`视图，您可以看到有一个新版本的应用程序类文件和相关的连接字符串列表。
...
最后一次，编译`OracleJavaSecure`并运行它。它第一次运行时，您会看到列表中已经有两个连接字符串(`connsHash.size = 2`)是从应用程序的前一版本复制过来的。在运行过程中，我们调用`removeAppConnString()`，然后运行`putAppConnections()`，用这个应用程序仅有的一个连接字符串列表更新 Oracle。第二次运行时，你会看到，因为我们没有更新和添加连接字符串，只有一个开始(`connsHash.size = 1`)。
...
我们得出结论，我们的`copyAppConnections()`流程将现有的连接字符串列表从应用程序的一个版本复制到下一个版本(在内部类中使用更新的`innerClassRevLvl`)。)当我们升级我们的应用程序并且不需要改变连接字符串时，这将节省时间。

### 从不同的应用程序 TestOracleJavaSecure 进行测试

`TestOracleJavaSecure`独立于运行应用程序验证的任何实例。它将作为一个客户端应用程序，调用`OracleJavaSecure`类中的方法来使用那些安全特性。使用`OracleJavaSecure`，测试应用程序将做以下事情:
...
编辑文件*chapter 11/testo js/testoraclejavasecure . Java*。更正[清单 11-58](#list_11_58) 中显示的`appusr`的密码。我们将把这个用户添加到`TestOracleJavaSecure`应用程序的连接字符串列表中。我们仍然使用`HRVIEW`作为我们的应用程序 ID，这意味着我们将作为`appusr`连接，并使用角色`hrview_role`(在`appsec.v_application_registry`中注册)。
...
*   调用`getAppConnections()`进行密钥交换，并获得该应用程序的连接字符串列表。
*   调用`putAppConnections()`来更新 Oracle 中的连接字符串列表。
*   调用`getAAConnRole()`解码并使用一个连接字符串得到一个`Connection.`

我们将以管理用户的身份编译并运行`TestOracleJavaSecure`(在我们的例子中，是一个具有`appver_admin`角色的用户，`OSADMIN`——可能是您的操作系统用户 ID)。)我们开始运行这个应用程序时没有双因素身份验证代码。以下是命令和结果:
...
对`putAppConnections()`的调用报告了一个`NullPointerException`——我们不会像捕获常规客户端应用程序调用的方法中的异常那样捕获管理命令中的异常。其他连接到 Oracle 的尝试指出需要返回双因素身份验证代码。
...
我们将测试我们对应用程序连接字符串更新的限制，仅限于被授予了`appver_admin`角色的用户。为此，您需要作为一个操作系统用户登录，该用户对应于一个没有`appver_admin`角色的 Oracle 用户，在我们的示例中，这个角色相当于`OSUSER`。再次运行`TestOracleJavaSecure`。您将会看到，当应用程序调用方法`putAppConnections()`时，抛出了一个关于`appsec_admin_pkg`包的异常。该用户没有执行从该方法调用的 Oracle 函数`f_set_decrypt_conns`的权限。
...
我们希望使用非管理用户演示其他非管理功能。为此，编辑*TestOracleJavaSecure.java*并注释调用`putAppConnections()`方法的代码行。然后重新编译并重新运行应用程序。您将看到以下成功结果:
...
*   我们建立了一个管理角色，限制谁可以更新应用程序的连接字符串。
*   我们将应用程序验证流程转移到一个新的、经过强化的 Oracle 数据库实例中，`apver.`
...
*   将应用程序连接字符串从早期版本复制到当前应用程序版本
*   向我们的应用程序连接字符串列表添加其他身份验证凭证，例如 FTP 密码
*   审查公众访问数据字典视图的弱点
