...
您可能感兴趣的最后一个技巧是:如果用户没有完成预期的活动就退出登录屏幕，那么我们将退出运行应用的整个 JVM。这是通过调用`System.exit()`的`this_WindowClosing()`方法来处理的，如[清单 12-9](#list_12_9) 所示。每当用户点击窗口右上角的 X 来关闭窗口时，`this_WindowClosing()`方法就会被自动调用。在进入一个应用的正常过程中，我们不会关闭`Login`屏幕，只是简单的使其不可见。
