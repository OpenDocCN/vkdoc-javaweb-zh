...
![images](img/square.jpg) **注**安全管理接口应用程序的文件可以在 *[第 12 章](#ch12) /orajavsec* 目录下找到。
...
![images](img/square.jpg) **注意**在`hr.v_emp_mobile_nos`视图中添加一个条目并不等同于使用我们在视图中输入的用户 ID 创建一个 Oracle 用户。但是，我们需要创建 Oracle 用户，以便该用户能够访问我们的应用程序。作为一种必然的情况，我们可以删除一个 Oracle 用户，但是`hr.v_amp_mobile_nos`中的条目不会被自动删除。
...
![images](img/square.jpg) **注意**能够通过`ojsadmin`代理的用户能够将任何应用分配给任何用户。也许另一个层次的过滤是合适的，但这并没有在本书中增加。从积极的一面来看，这些`ojsadmin`管理员可以互相代替，分配用户对彼此应用程序的访问权。我们在这里进行的唯一特定于应用程序的管理过滤是限制哪些管理员可以更改哪些应用程序的连接字符串(密码)。
...
![images](img/square.jpg) **注意**为了注册应用程序，该应用程序内部类必须以编译形式存在于运行注册新应用程序屏幕的同一客户端上。应用程序内部类也必须位于当前的类路径中。
...
![images](img/square.jpg) **注意**我们还没有完成这个新应用的双因素认证和密钥交换。当我们用一个空的连接列表调用`putAppConnections()`时，Oracle 数据库并不回避解密和加密，而是为新应用程序插入一个空的连接字符串列表。
...
![images](img/square.jpg) **注意**当新的运行时进程启动时，登录对话框将使用新的应用程序身份再次实例化。这将导致输入双因素身份验证代码的新提示，一个特定于所选应用程序的代码。
