...
创建新的 Vaadin 项目有多种方法。Vaadin建议使用在 [`https://start.vaadin.com`](https://start.vaadin.com) 可用的工具来创建新项目。不幸的是，在撰写本文时，这个在线工具不包括创建没有生成视图和 Spring Boot 的最小项目的选项。当你读这本书的时候，这可能会改变，但是为了你的方便，我已经创建了一个最小的*空项目*，你可以用它作为模板开始编写你的应用。您可以在本书附带的源代码的 *ch02/empty-project/* 目录中找到这个空项目(参见 [`www.apress.com/9781484271780`](http://www.apress.com/9781484271780) 处的说明)。
