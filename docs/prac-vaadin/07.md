...
一旦我们进入登录视图，输入有效的凭证(例如，*用户* / *用户*，并在浏览器中手动请求*空的*视图( *http://localhost:8080/* )，我们看到一个新的视图(用户)可用(参见图 [7-3](#Fig3) )。Vaadin现在知道了这个观点。它不知道管理视图，而这正是我们想要的——我们想要限制对该视图的访问。如果你尝试请求管理视图，Vaadin 将不能显示它，因为它还没有被设置。
