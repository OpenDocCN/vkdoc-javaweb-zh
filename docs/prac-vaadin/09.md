...
很容易认为 *script.js* JavaScript 文件只有在浏览器请求`JavascriptIntegrationView`时才被加载。然而，正如您在图 [9-7](#Fig7) 的截图中看到的，当您请求应用的任何视图时，该文件被加载(截图中默认的*空*视图)。如果您记得文件被编译成一个包，这就很容易理解了。
