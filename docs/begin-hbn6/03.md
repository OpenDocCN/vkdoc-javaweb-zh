...
在应用服务器(如 WildFly、GlassFish 或 Geronimo)中，通过资源注入访问持久性 API 应用部署者为 Java 持久性 API 配置一个上下文，应用自动获取一个`EntityManager`(相当于`Session`的 JPA)。将 Hibernate 配置为 JPA 提供者是完全可能的(也可能是更好的);然后，您可以使用 Hibernate APIs，并将其转换为`Session`。
