...
现在您已经看到了应用程序的目录结构，您将了解在测试 URL:[http://localhost:9000](http://localhost:9000)时默认的欢迎页面是如何显示的。
