...
在本节中，您将使用支持 JSF2 的 Eclipse 3.8 或更新版本创建一个简单的 Hello World JSF web 应用程序。 *x* 。通过选择文件![image](img/arrow.jpg)新建![image](img/arrow.jpg)项目![image](img/arrow.jpg) web ![image](img/arrow.jpg)动态 Web 项目，创建如图[图 6-3](#Fig3) 所示的动态 Web 项目。在目标运行时中指定 Apache Tomcat v7.0，在配置中选择 JavaServer Faces 项目，然后单击 Next。
