...
Quarkus 中的依赖注入机制基于 CDI(contexts and dependency injection ),包括一个扩展框架，用于扩展功能以及配置、引导和集成框架到您的应用程序中。添加扩展就像添加 Maven 依赖项一样简单。
...
![img/509649_1_En_3_Figad_HTML.gif](img/509649_1_En_3_Figad_HTML.gif)验证注释和`@Column`中定义的约束之间的区别在于，验证注释是应用程序范围的，而约束是数据库范围的。
