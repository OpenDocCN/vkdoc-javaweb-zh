...
`JpaRepository`延伸`PagingAndSortingRepository`，?? 又延伸`CrudRepository`。他们的主要职能是:
...
本章的最后一部分将向您展示如何定制应用横幅，就像在 Spring Boot 所做的那样。这个操作在 Quarkus 非常容易。首先，你需要在`src/main/resources`中创建一个`banner.txt`文件，如清单 3-1 所示。
...
*   [1]在 [` docs 了解更多关于`@MappedSuperclass`的信息。jboss。org/ hibernate/ orm/ 5。4/User Guide/html _ single/Hibernate _ User _ Guide。html #实体-继承-映射-超类`](https://docs.jboss.org/hibernate/orm/5.4/userguide/html_single/Hibernate_User_Guide.html%2523entity-inheritance-mapped-superclass)
...
*   [3]在 [` docs 了解有关验证约束的更多信息。jboss。org/ hibernate/ validator/ 7。0/reference/en-US/html _ single/# _ validating _ constraints`](https://docs.jboss.org/hibernate/validator/7.0/reference/en-US/html_single/%2523_validating_constraints)