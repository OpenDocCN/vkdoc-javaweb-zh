...
在开始处理应用程序代码之前，您需要将![img/509649_1_En_12_Figa_HTML.gif](img/509649_1_En_12_Figa_HTML.gif) PostgreSQL 数据库和 Keycloak 放到 Kubernetes 集群中。
...
清单 [12-3](#PC4) 显示了用于请求 PostgreSQL 存储访问的`PersistentVolumeClaim`。
...
在将![img/509649_1_En_12_Figk_HTML.gif](img/509649_1_En_12_Figk_HTML.gif) PostgreSQL 数据库和 Keycloak 引入 Kubernetes 之后，您可以继续将 monolithic QuarkuShop 应用程序部署到 Kubernetes。这个练习对于学习如何只将一个应用程序部署到 Kubernetes 非常有用。
...
为了验证我说的是事实，![img/509649_1_En_12_Figl_HTML.gif](img/509649_1_En_12_Figl_HTML.gif)让我们构建应用程序并检查它:
...
![img/509649_1_En_12_Figm_HTML.gif](img/509649_1_En_12_Figm_HTML.gif)注意，为了可见性，我们在 Keycloak 服务的`service name`后面附加了`namespace name`，但是我们没有为 PostgreSQL 服务这样做。这是因为在我们部署应用程序的`default`名称空间中存在 PostgreSQL 服务。Keycloak 服务在`keycloak`名称空间中可用。
...
接下来，您启用 Kubernetes `ConfigMap`访问，并告诉应用程序哪个`ConfigMap`具有您需要的属性:
