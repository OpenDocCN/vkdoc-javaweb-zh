...
下一步是指定一个事务管理器供 Spring 使用。由于您要处理的是单个数据库，所以清单 9-6 中显示的 org . spring framework . JDBC . data source . data source eTransactionManager 非常适合您的情况。从[清单 9-6](#list6) 中，您可以看到 data sourcetransactionmanager 需要一个数据源来获取和管理到数据库的连接。
