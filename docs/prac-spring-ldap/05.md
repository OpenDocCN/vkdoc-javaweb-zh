...
[图 5-1](#Fig1) 描述了当一个应用与一个对象工厂一起执行 LDAP 操作时所涉及的流程。流程从应用调用搜索或查找操作开始。JNDI API 将执行请求的操作，并从 LDAP 中检索条目。这些结果然后被传递给注册的对象工厂，后者将它们转换成对象。这些对象被移交给应用。
...
![image](img/sq.jpg) **注**比较[清单 5-12](#list12) 中的代码和[清单 3-10](#list10) 中的代码。您可以清楚地看到，DirContextAdapter 在简化属性操作方面做得非常好。
