...
![image](images/sq.jpg) **注意**根 DSE 或 DSA 特定条目是 LDAP 服务器中的一个特殊条目。它通常保存特定于服务器的数据，如供应商名称、供应商版本以及它支持的不同控件和功能。

范围参数

scope 参数确定需要执行的 LDAP 搜索相对于基准的深度。LDAP 协议定义了三种可能的搜索范围:基本、一级和子树。[图 6-1](#Fig1) 显示了在不同搜索范围内被评估的条目。

![9781430263975_Fig06-01.jpg](images/9781430263975_Fig06-01.jpg)
...
![image](images/sq.jpg) **注意**操作属性的例子包括 createTimeStamp 和 pwdAccountLockedTime，前者保存条目创建的时间，后者记录用户帐户被锁定的时间。
...
![9781430263975_Fig06-02.jpg](images/9781430263975_Fig06-02.jpg)

[图 6-2](#_Fig2) 。过滤器层次结构

![image](images/sq.jpg) **注意**默认情况下，大多数 LDAP 属性值在搜索时不区分大小写。
