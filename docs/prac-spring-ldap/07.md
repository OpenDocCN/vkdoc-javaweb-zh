...
下一步是向 AbstractFallbackRequestAndResponseControlDirContextProcessor 提供加载控件所需的信息。abstractfallbackrequestandresponsecontroldicontextprocessor 需要来自子类的两条信息:要使用的请求和响应控件的完全限定类名，以及应该用作后备的控件的完全限定类名。[清单 7-7](#_list7) 显示了完成这项工作的构造器代码。
...
Spring LDAP 抽象了清单 7-10 中的大部分代码，并使用 PagedResultsDirContextProcessor 简化了页面控件的处理。[清单 7-11](#list11) 显示了 Spring LDAP 代码。
