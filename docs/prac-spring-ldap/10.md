...
在执行上述 authenticate 方法期间发生的任何异常将被传递给作为方法参数提供的 AuthenticationErrorCallback 实例。这个收集的异常可以被记录或用于身份验证后的过程。[清单 10-6](#list6) 和[清单 10-7](#list7) 分别展示了 AuthenticationErrorCallback API 及其简单实现。回调中的 execute 方法可以决定如何处理引发的异常。在您的简单实现中，您只是存储它并让它对 LdapTemplate 的搜索调用者可用。
