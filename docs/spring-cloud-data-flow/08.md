...
清单 8-2 显示的是`docker-compose-kafka.yml`文件；注意和 RabbitMQ 差不多，用的是 Kafka 和 Zookeeper。还要检查属性并确定您使用的是服务名。
...
请注意，应用是使用这种模式命名的:`<stream-name>.<app>-<version>`。在这种情况下，你有“??”和“??”。史奇普干的。它会跟踪版本。如果您取消部署并重新部署(停止并重新启动)流，您应该看到版本增加到`v2`。
