...
截至 2020 年初，从 JEE 8 到 Jakarta EE 8 的过渡仍在进行中。所以取决于你什么时候读这本书，它仍然可能是关于Jakarta EE 8 的在线研究，你必须查阅关于 JEE 8 和Jakarta EE 8 的页面。这是你应该记住的事情。为了在本书中保持简单，我们将只讨论 Jakarta EE。

写这本书的时候，发布的Jakarta EE 8 服务器还不多。基本上有以下几种:
...
表 [2-2](#Tab2) 给出了从服务器查询各类信息的子命令。您在`bin/asadmin list-applications`中输入它们(显然，如果您还没有安装任何应用，列表将是空的)。
...
*   **练习 1:** 描述 JSE 和Java EE/Jakarta EE 的关系。
