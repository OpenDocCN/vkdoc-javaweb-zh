...
例如，我们从第 4 章[的](04.html)中恢复了`HelloWorld`应用，并为 AJAX 请求添加了第二个表单和一个显示 AJAX 调用结果的区域。
...
*   **练习 1:[4](04.html)的`HelloWorld`应用中的**，首先确保使用了一个模型类`UserData`，然后添加一个名为`age`的新整数字段。更新视图中的表单，并添加一个名为“你的年龄是多少？”。更新控制器，并应用 bean 验证约束，确保用户输入的年龄大于零。添加错误处理，如第 [6](06.html) 章所述。还可以通过添加年龄来更新响应页面。

*   **练习 2:** 在第 [4](04.html) 章的`HelloWorld`应用中，将会话注入控制器。在控制器的`showIndex()`方法中，将会话 ID 写入`System.err`。

*   **练习 3:** 在第 [4](04.html) 章的`HelloWorld`应用中，将头注入到`greeting()`方法中。将所有请求头写入`System.err`。
...
*   **练习 5:** 在第 [4](04.html) 章的`HelloWorld`应用中，编写一个计算控制器响应时间的观察器。将结果输出到`System.err`。
