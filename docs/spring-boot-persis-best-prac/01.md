...
完整的应用可在 GitHub <sup>[9](#Fn9)</sup> 上获得。
...
完整的应用可在 GitHub <sup>[15](#Fn15)</sup> 上获得。
...
在这种情况下，评论可以与一本书、一本杂志或一篇文章相关联。通过 Bean 验证 <sup>[17](#Fn17)</sup> 可以在应用级别实现这个约束。首先定义一个将在类级别添加到`Review`实体的注释:
