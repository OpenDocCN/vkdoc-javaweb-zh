...
检查粗体行(`authorRepository.save(author)`)——需要这一行吗？正确答案是否定的！当应用从数据库中获取`author`时，它就成为一个托管实例。这意味着如果实例被修改，Hibernate 将负责触发`UPDATE`语句。这是通过休眠脏检查机制在刷新时完成的。换句话说，可以通过以下方法实现相同的行为:
