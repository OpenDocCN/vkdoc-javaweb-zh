...
这一次，应用程序日志显示您已经消除了长时间运行的事务。当`postReview()`调用被提交和关闭时，事务开始(附加的数据库连接在连接池中返回), string-response 被立即发送给检查者。事件处理程序的执行是异步的，需要一个新的线程和一个新的事务。数据库连接的获取被延迟到真正需要的时候(这时应该更新书评状态)。因此，图书检查不会免费保持任何数据库连接活动/繁忙。
