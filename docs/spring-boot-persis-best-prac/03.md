...
完整的应用可在 GitHub <sup>[1](#Fn1)</sup> 上获得。
...
这三种方法都触发相同的 SQL `SELECT`,并受益于会话级可重复读取。完整的应用可在 GitHub <sup>[3](#Fn3)</sup> 上获得。
...
完整的应用可在 GitHub <sup>[23](#Fn23)</sup> 上获得。
...
图 [3-5](#Fig5) 中的时间-性能趋势图是针对 MySQL 在具有以下特征的 Windows 7 机器上获得的:英特尔 i7、2.10GHz 和 6GB RAM。应用和 MySQL 运行在同一台机器上。
...
图 [3-7](#Fig7) 中显示的时间-性能趋势图是针对 MySQL 在具有以下特征的 Windows 7 机器上获得的:英特尔 i7、2.10GHz 和 6GB RAM。应用和 MySQL 运行在同一台机器上。

完整的应用可在 GitHub <sup>[28](#Fn28)</sup> 上获得。
...
图 [3-9](#Fig9) 中显示的时间-性能趋势是在具有以下特征的 Windows 7 机器上针对 MySQL 获得的:英特尔 i7、2.10GHz 和 6GB RAM。应用和 MySQL 运行在同一台机器上。

完整的应用可在 GitHub <sup>[29](#Fn29)</sup> 上获得。
...
GitHub <sup>[36](#Fn36)</sup> 上有源代码。如果你不想使用`{` `EntityName` `}.{` `RepositoryMethodName` `}`约定，而更喜欢`@Query(name="...")`，那么就来看看这个应用 <sup>[37](#Fn37)</sup> 。此外，您更喜欢基于`orm.xml`的 XML 方法，这个应用 <sup>[38](#Fn38)</sup> 就是为您准备的。

如果(出于任何原因)目标必须通过`EntityManager`直接完成，而不需要`@NamedNativeQuery`，那么考虑这个 <sup>[39](#Fn39)</sup> 。

### 实体映射

您可以通过`EntityResult`获取单个实体或多个实体。GitHub <sup>[40](#Fn40)</sup> 上有完整的启动应用。或者，如果你不想依赖于`{EntityName}.{RepositoryMethodName}`约定，而更喜欢`@Query(name="...")`，那么就来看看这个应用 <sup>[41](#Fn41)</sup> 。
...
完整的应用可在 GitHub <sup>[47](#Fn47)</sup> 上获得。
...
目标是只获取所有作者的`name`和`age`。这一次，应用依赖于 Blaze Persistence<sup>[49](#Fn49)</sup>实体视图。Blaze Persistence 是一个开源项目，旨在为 JPA 提供者提供丰富的标准 API。由于在 Spring Boot 之外，它必须作为一个依赖项添加到应用中。例如，通过 Maven，您可以向`pom.xml`添加以下依赖项:
...
有时这根本不是问题。比如图 [3-16](#Fig16) 的输出就没问题。如何避免结果集被截断？如果这是您的应用设计的一个要求，该怎么办呢？
...
根据经验，使用本机查询来编写复杂的查询。这样，您可以利用窗口函数、常用表表达式(CTE)、`PIVOT` <sup>[64](#Fn64)</sup> 等等。在适当的情况下使用原生查询可以大大提高应用的性能。不要忘记分析您的 SQL 查询和执行计划，以优化它们的结果。
...
请注意，这是原始结果。它可以在存储器中被进一步处理，以赋予它不同的形状(例如，将一个作者的所有书籍分组在一个列表下)。这一次，没有使用`Pageable`，也没有可用的页面元数据，但是您可以很容易地添加一些信息(例如，通过调整查询以获取由`DENSE_RANK()`分配的最大值，您可以获得作者的总数)。完整的应用可在 GitHub <sup>[65](#Fn65)</sup> 上获得(每个查询由`BookstoreController`中的 REST 端点公开)。
...
完整的代码可以在 GitHub <sup>[66](#Fn66)</sup> 上找到。这个应用也包含了`useCursorFetch=true`案例。
...
完整的应用可在 GitHub <sup>[67](#Fn67)</sup> 上获得。
...
仅此而已！完整的应用可在 GitHub <sup>[68](#Fn68)</sup> 上获得。
