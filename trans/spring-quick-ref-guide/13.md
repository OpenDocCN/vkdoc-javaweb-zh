...
![img/498572_1_En_13_Figa_HTML.jpg](img/498572_1_En_13_Figa_HTML.jpg)SpringCloud集成
...
您还可以使用`@IntegrationComponentScan`来扫描类路径，寻找特定于 Spring Integration的注释，比如`@MessagingGateway`注释。
...
默认情况下，如果存在一个千分尺`meterRegistry` bean，这将是一个包括Spring致动器的 Spring Boot 项目的情况，Spring Integration度量将由千分尺管理。如果您希望使用遗留的 Spring Integration 指标，可以向应用上下文添加一个`DefaultMetricsFactory`(来自 Spring Integration) bean。
