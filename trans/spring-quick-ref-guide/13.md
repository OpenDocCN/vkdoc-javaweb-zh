...
默认情况下，如果存在一个千分尺`meterRegistry` bean，这将是一个包括Spring致动器的 Spring Boot 项目的情况，Spring集成度量将由千分尺管理。如果您希望使用遗留的 Spring Integration 指标，可以向应用上下文添加一个`DefaultMetricsFactory`(来自 Spring Integration) bean。
