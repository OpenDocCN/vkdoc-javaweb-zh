...
![img/300017_2_En_1_Figb_HTML.jpg](img/300017_2_En_1_Figb_HTML.jpg)我们建议您将`JAVA_HOME`环境变量设置为指向 Java 14 的安装目录(JDK 解压缩的目录)，并将`$JAVA_HOME/bin (%JAVA_HOME%\bin`添加到系统的常规路径中(Windows 用户使用`)`)。这背后的原因是为了确保用 Java 编写的任何其他开发应用程序都使用这个版本的 Java，并防止开发过程中出现奇怪的不兼容错误。如果您想从终端运行构建，您肯定使用了预期的 Java 版本。
