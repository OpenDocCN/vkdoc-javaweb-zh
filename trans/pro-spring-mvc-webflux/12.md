...
如果凭证得到验证，就会创建一个`Authentication`实例，并存储在应用的`security context`中。`SecurityContext`包含在`SecurityContextHolder,`中，T3 是春天安全的核心。
...
`@Secured`是初始安全注释，不支持 SpEL 表达式。`@PreAuthorize`是春天安全 3 中新增的，功能强大。`@PreAuthorize`支持使用 SpEL 声明访问规则，因此您可以尽情发挥创造力。
