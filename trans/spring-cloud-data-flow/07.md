# 七、Spring CloudStream绑定器

在前一章中，我向您展示了 Spring Cloud Stream 应用启动器如何作为独立的应用工作，可以轻松交付企业级解决方案。我向您展示了 Spring Cloud Stream 模型应用，以及主内核如何基于 Spring Integration 和 Spring Boot 实现简单的配置。您看到了如何使用 Spring Cloud 函数和 Spring Integration非常容易地创建流。我还向您展示了一个特性(在我看来是最好的),叫做 *binder* ,它允许您的解决方案使用任何物理目的地或任何中间件消息代理。本章介绍如何创建自定义活页夹。
