...
在这一节中，我将通过创建简单的示例来介绍连接到 Spring 数据流服务器的所有可用方法，在接下来的章节中，您可以使用任何工具来实现复杂的解决方案。让我们首先启动并运行 Spring CloudStream的基础设施(数据流服务器、Skipper 服务器、MySQL、RabbitMQ 或 Kafka );您可以使用 Docker Compose 或 Kubernetes。同样，让我们定义我们的管道 DSL。
...
Spring 数据流外壳的另一个好处是它解析 DSL 的方式。您必须选择一种有效的方法来添加您的表达式，而不是组合它们。例如，`filter`应用需要传递一个表达式参数(正如您之前看到的`cURL` / `Httpie`命令)，并且您必须选择以下任意一个。
