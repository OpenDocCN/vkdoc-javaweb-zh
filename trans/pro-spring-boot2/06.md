...
在这一章中，我将向你展示 Spring Framework 5 的最新补充，以及如何在 Spring Boot 中使用它。为 web 应用构建的新的反应堆栈是 Spring WebFlux，它是在 Spring 框架的 5.0 版本中添加的。它是一个完全非阻塞的框架，依赖于 Project Reactor，支持 reactive streams back pressure，运行在 Netty 和 Undertow 等服务器以及 Servlet 3.1+容器上。
...
记住 Spring WebFlux 使用函数式编程来创建反应式 web 应用。让我们创建声明路由函数的`ToDoRouter`类(参见清单 [6-7](#PC15) )。
...
对于这个 ToDo 应用，我们将使用函数式编程。让我们在`ToDoRouter`类中创建路由器和处理器(参见清单 [6-11](#PC21) )。
