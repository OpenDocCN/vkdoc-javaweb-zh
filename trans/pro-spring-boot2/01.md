...
也许你在想，“我想学 Spring Boot。为什么我需要了解 Spring Framework？”嗯，让我告诉你，Spring Boot 的春天是。Spring Boot 有不同的机制来运行 Spring 应用；要理解 Spring Boot 真正是如何工作的，有必要了解更多关于 Spring 框架的知识。
...
接下来，我们从弹簧配置开始。Spring 需要开发人员决定类在哪里，它们如何相互交互，以及 web 应用的一些额外配置。
...
这里没有必要声明映射的类或连接，因为 Spring 数据模块会处理它；你只需要声明一个`persistence-unit`名。
...
接下来，让我们创建一个包含所有 CRUD 操作的存储库。在这里，应用使用了 Spring 数据模块的强大功能，它隐藏了所有带有表和 keep 会话的样板映射类，甚至还进行事务处理。Spring 数据实现了所有的 CRUD 换句话说，您不需要担心如何保存、更新、删除和查找记录。
