...
在 3.0 版本之后，Spring 框架创建了不同的团队，专门研究不同的技术。Spring数据项目组诞生了。这个项目的目标是简化数据访问技术的使用，从关系数据库和非关系数据库，到 map-reduce 框架和基于云的数据服务。这个 Spring 数据项目是特定于给定数据库的子项目的集合。
...
## Spring Data Redis(Spring数据重定向器)
