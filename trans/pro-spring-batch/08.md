...
Spring批次已经覆盖了你。告诉 Spring Batch 不要继续处理某个项目实际上非常容易。为此，ItemProcessor 不返回项目，而是返回 null。因此，在这种情况下，如果您在数据库中找不到客户，您将希望 CustomerItemProcessor 返回 null，以便 AccountExecutiveItemProcessor 不会因为没有`Customer`来查找而引发异常。更新后的代码如清单 8-23 所示。
