...
![images](images/square.jpg) **注意**item processor 返回的类型不需要与它接受的输入类型相同。
...
![images](images/square.jpg) **注意**Spring 批处理框架中包含的验证器接口与作为核心 Spring 框架一部分的验证器接口不同。Spring Batch 提供了一个适配器类`SpringValidator`来处理这些差异。
