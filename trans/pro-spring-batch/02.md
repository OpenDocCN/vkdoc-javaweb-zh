...
*   灵活性:仅仅通过修改 XML 就能改变处理顺序的能力是许多框架都在谈论的，但很少有人实现。Spring批次是一个交付。想想之前的银行账户例子。，如果您想在贷记之前应用借记，唯一需要的更改是重新排序作业 XML 中的步骤([第 4 章](04.html#ch4)给出了一个例子)。您还可以跳过一个步骤，根据上一个步骤的结果有条件地执行一个步骤，甚至只需调整 XML 就可以并行运行多个步骤。
