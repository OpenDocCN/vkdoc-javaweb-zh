...
![images](images/square.jpg) **注意**传入一个已经被引用了作业的配置会导致抛出`DuplicateJobException`。
...
![images](images/square.jpg) **注意**如果作业的`ExitStatus`为`COMPLETE`，那么无论您是否将所有步骤配置为`allow-start-if-complete="true"`，JobInstance 都无法重新运行。
