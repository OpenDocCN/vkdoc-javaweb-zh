...
`Jackson2JsonMessageConverter`使用了 AMQP Spring预先配置的杰克逊的`ObjectMapper`。然后，`RabbitTemplate`实现将使用我们的 bean，这个类序列化对象并将对象作为 AMQP 消息发送给代理。在订户端，我们可以受益于 JSON 格式的流行，使用任何编程语言反序列化内容。我们也可以使用自己的对象表示，忽略消费者端不需要的属性，从而减少微服务之间的耦合。如果发布者在有效负载中包含新字段，订阅者不需要做任何更改。
